    ...
    (function ($) {
        ...
        const getMD5DataList = async () => {
            ...
            try {
                ...
                // 提取数组部分: 匹配 '=>' 后面的第一个 '[' 到最后一个 ']'
                const match = text.match(/=>\s*(\[[\s\S]*\])/);
                if (match && match[1]) {
                    // 使用 new Function 安全地解析数组字符串
                    const data = new Function('return ' + match[1])();
                    if (Array.isArray(data)) {
                        console.log(`✅ 动态题库获取成功！共加载 ${data.length} 条题目数据`);
                        return data;
                    }
                }
                throw new Error('解析题库格式失败');
            } catch (error) {
                clearTimeout(timeoutId);
                if (error.name === 'AbortError') {
                    console.warn('⚠️ 动态题库获取超时（10秒），切换到本地题库');
                } else {
                    console.warn('⚠️ 动态题库获取失败，切换到本地题库。错误原因:', error.message);
                }
                console.log(`📦 使用本地备份题库，共 ${fallbackMD5Data.length} 条数据`);
                return fallbackMD5Data;
            }
        };
        ...
        const renderResultInExamStartPage = async () => {
            ...
            if (!location.href.includes('exam.kaoshixing.com/exam/exam_start')) {
                console.log('当前页面不是考试页面，跳过答题');
                return;
            }

            // 先等待题库加载完成
            console.log('⏳ 等待题库加载...');
            const md5DataList = await getMD5DataList();
            console.log('📝 题库已就绪，开始匹配答案...');
            ...
            $('.question-content').find('.question-name .pre-wrap').each(function () {
                totalCount++;
                // 题干
                let questionName = replaceText($(this).text());
                ...
                $(this).parents('.question-content').find('.answers .select').each(function () {
                    ...
                    let answerMD5 = md5(replaceText(text));
            ...
            console.log(`🎯 题目匹配完成: ${matchedCount}/${totalCount} 题匹配成功，${totalCount - matchedCount} 题未匹配`);
        }
         ...
         * 未查询到的答案 随机选择
         */
        const renderNotFindQuestion = () => {
            window._notFindQuestionInterval = setInterval(() => {
        ...
        const buttonControl = () => {
            ...
            console.log(`⏱️ 将在 ${second/1000} 秒后自动交卷`);
        ...
        // 停止脚本的函数（挂载到window方便手动调用）
        window.stopAutoExam = function() {
            if (window._notFindQuestionInterval) {
                clearInterval(window._notFindQuestionInterval);
                console.log('已停止随机选择功能');
            }
        };

        // 执行主逻辑
        (async function main() {
            console.log('===== 答题脚本开始运行 =====');

            // 1. 先等题库加载完毕 & 匹配答案
            await renderResultInExamStartPage();

            // 2. 匹配完之后，再对未匹配的题随机选
            renderNotFindQuestion();
